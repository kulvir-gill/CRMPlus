...
interface Quote { id: number; quoteNumber: string; accountName: string; status: string; total: number; createdAt: string; validUntil?: string }
interface Account { id: number; name: string }

const statusColor: Record<string, string> = { Draft: 'bg-gray-100 text-gray-700', Sent: 'bg-blue-100 text-blue-700', Accepted: 'bg-green-100 text-green-700', Rejected: 'bg-red-100 text-red-700', Expired: 'bg-orange-100 text-orange-700' }

export default function Quotes() {
  const [quotes, setQuotes] = useState<Quote[]>([])
  const [accounts, setAccounts] = useState<Account[]>([])
  const [showModal, setShowModal] = useState(false)
  const [form, setForm] = useState({ accountId: '', status: 'Draft', validUntil: '', notes: '', taxRate: '0', lines: [{ description: '', quantity: '1', unitPrice: '0' }] })
  ...
  const save = async () => {
    await api.post('/quotes', {
      ...
      validUntil: form.validUntil || null, notes: form.notes, taxRate: parseFloat(form.taxRate),
      lineItems: form.lines.map((l) => ({ description: l.description, quantity: parseFloat(l.quantity), unitPrice: parseFloat(l.unitPrice) }))
    })
    setShowModal(false); load()
  }

  const remove = async (id: number) => { if (!confirm('Delete?')) return; await api.delete(`/quotes/${id}`); load() }

  return (
    <div>
      <PageHeader title="Quotes" subtitle={`${quotes.length} quotes`}
        action={<button onClick={() => setShowModal(true)} className="bg-indigo-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-indigo-700">+ New Quote</button>} />
      <div className="p-6">
        <div className="bg-white rounded-xl border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 text-xs uppercase tracking-wider">
              <tr>{['Quote #', 'Account', 'Status', 'Total', 'Valid Until', 'Created', ''].map((h) => <th key={h} className="px-4 py-3 text-left">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y">
              {quotes.map((q) => (
                <tr key={q.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColor[q.status] ?? ''}`}>{q.status}</span></td>
                  ...
                  <td className="px-4 py-3 text-gray-600">{new Date(q.createdAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3 text-right"><button onClick={() => remove(q.id)} className="text-red-500 hover:underline">Delete</button></td>
                </tr>
              ))}
              {quotes.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No quotes</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
